...
//	}
	static {
		...
		smag.put(HullSize.DESTROYER, 1500f);
		smag.put(HullSize.CRUISER, 2000f);
		smag.put(HullSize.CAPITAL_SHIP, 2500f);
	}
...
//		stats.getSensorStrength().modifyFlat(id, bonus);
		if (stats.getVariant().getSMods().contains("hiressensors") || (Global.getSettings().getBoolean("BuiltInSMod") && stats.getVariant().getHullSpec().isBuiltInMod("hiressensors"))) {
			stats.getSightRadiusMod().modifyFlat(id, (Float) smag.get(hullSize));
		}
...
//		if (index == 2) return "" + ((Float) mag.get(HullSize.CRUISER)).intValue();
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		if (isForModSpec) {
			...
			tooltip.addPara("Increases the ship's in-combat vision range by %s/%s/%s/%s, based on hull size.", 10f, Misc.getGrayColor(), Misc.getHighlightColor(), "1000","1500","2000","2500");
			return;
		} else if (ship.getVariant().getSMods().contains("hiressensors")) {
			tooltip.addSectionHeading("S-mod bonus", Misc.getStoryOptionColor(), Misc.getStoryDarkColor(), Alignment.MID, 10f);
			tooltip.addPara("Increases the ship's in-combat vision range by %s/%s/%s/%s, based on hull size.", 10f, Misc.getPositiveHighlightColor(), Misc.getHighlightColor(), "1000","1500","2000","2500");
		} else if (Global.getSettings().getBoolean("BuiltInSMod") && ship.getHullSpec().isBuiltInMod("hiressensors")) {

	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		if (stats.getVariant() != null) {
			mod = Math.max(stats.getVariant().getHullSpec().getCargo() * MIN_FRACTION, mod);
                ...
                if (stats.getVariant().getSMods().contains("expanded_cargo_holds") || (Global.getSettings().getBoolean("BuiltInSMod") && stats.getVariant().getHullSpec().isBuiltInMod("expanded_cargo_holds"))) {
                    mod *= 2;
                }
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
        ...
        public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
                     if (isForModSpec) {
                             ...
                             tooltip.addPara("Doubles the cargo capacity increase and, for civilian hulls, negates the maintenance cost increase.", Misc.getGrayColor(), 10f);
                             return;
                     } else if (ship.getVariant().getSMods().contains("expanded_cargo_holds")) {
                             tooltip.addSectionHeading("S-mod bonus", Misc.getStoryOptionColor(), Misc.getStoryDarkColor(), Alignment.MID, 10f);	
                             tooltip.addPara("Doubles the cargo capacity increase and, for civilian hulls, negates the maintenance cost increase.", Misc.getPositiveHighlightColor(), 10f);
                     } else if (Global.getSettings().getBoolean("BuiltInSMod") && ship.getHullSpec().isBuiltInMod("expanded_cargo_holds")) {
